    ...
    <header className="sticky top-0 z-50 border-b border-white/5 bg-[#0a0a0a]/80 backdrop-blur-xl">
      <div className="mx-auto flex h-16 max-w-7xl items-center justify-between gap-4 px-4 sm:px-6 lg:px-8">
        ...
        <nav className="hidden items-center gap-8 md:flex">
          {NAV_LINKS.map((link) => {
            const isTranslator = link.label === "Translator";
            const isPhotoRestoration = link.label === "Photo Restoration";
            const onPhotoRestorationPage = pathname === "/photo-restoration";

            const isActive = isTranslator
              ? hash === "#translator" || pathname === "/translator"
              : isPhotoRestoration
                ? onPhotoRestorationPage && hash !== "#translator"
